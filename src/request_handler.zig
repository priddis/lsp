...
pub fn recv() UnrecoverableError!void { 
    ...
    const parsed_json = std.json.parseFromSlice(lsp_messages.LspRequest, allocator, json_buf[0..length], .{ .allocate = .alloc_always }) 
        catch |err| return logger.throw("Could not parse request {!}", .{err}, UnrecoverableError.CouldNotParseRequest);
...
fn handle(req: lsp_messages.LspRequest) ?[]const u8 {
    const lsp_method = std.meta.stringToEnum(lsp_messages.LspMethod, req.method) orelse {
        logger.err("unrecognized method {s}\n", .{req.method});
        return null;
    };
    logger.err("method = {s}\n", .{req.method});
...
fn parseLspHeader(alloc: std.mem.Allocator, reader: anytype) UnrecoverableError!usize {
    ...
    if (colon_opt == null) {
        logger.err("No colon found in content length {s}\n", .{content_length_line});
    ...
    if (!std.mem.eql(u8, content_length_key, "Content-Length")) {
        logger.err("Key does not contain string  Content-Length {s}\n", .{content_length_line});
        return UnrecoverableError.CouldNotParseHeader;
    }
    const content_length: usize = std.fmt.parseInt(usize, content_length_value, 10) catch {
        logger.err("Error parsing content length from string {s}\n", .{content_length_value});
        return UnrecoverableError.CouldNotParseHeader;
    };
